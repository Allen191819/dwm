...
static const Rule rules[] = {
    /* class                 instance              title             tags mask
       isfloating  isfullscreen  monitor */
...
static Key keys[] = {
    ...
    {MODKEY | ControlMask,
     XK_comma,
     setmfact,
     {.f = -0.05}}, /* super ctrl ,       |  缩小主工作区 */
    {MODKEY | ControlMask,
     XK_period,
     setmfact,
     {.f = +0.05}}, /* super ctrl .       |  放大主工作区 */
    ...
    {MODKEY | ShiftMask,
     XK_space,
     selectlayout,
     {.i = +1}}, /* super shift space  |  在网格和主次栈模式中切换 */
    ...
    TAGKEYS(XK_1, 0, 0, 0) TAGKEYS(XK_2, 1, 0, 0) TAGKEYS(
        XK_3, 2, 0, 0) TAGKEYS(XK_4, 3, 0, 0) TAGKEYS(XK_5, 4, 0,
                                                      0) TAGKEYS(XK_6, 5, 0, 0)
        TAGKEYS(XK_7, 6, "~/scripts/app-starter.sh pycharm", 0) TAGKEYS(
            XK_8, 7, "~/scripts/app-starter.sh idea",
            0) TAGKEYS(XK_9, 8, "~/scripts/app-starter.sh pavucontrol", 0)
            TAGKEYS(XK_c, 9, "~/scripts/app-starter.sh browser", 0) TAGKEYS(
                XK_p, 10, "~/scripts/app-starter.sh music",
                0) TAGKEYS(XK_n, 11, "~/scripts/app-starter.sh pcmanfm", 0)
                TAGKEYS(XK_u, 12, "~/scripts/app-starter.sh tim", 0) TAGKEYS(
                    XK_w, 13, "~/scripts/app-starter.sh wechat", 0)
                    TAGKEYS(XK_z, 14, "~/scripts/app-starter.sh qq", 0) TAGKEYS(
                        XK_o, 15, "~/scripts/app-starter.sh virt",
                        0) TAGKEYS(XK_v, 16, "~/scripts/app-starter.sh db", 0)
                        TAGKEYS(XK_g, 17, "~/scripts/app-starter.sh feishu", 0)
                            TAGKEYS(XK_a, 18,
                                    "~/scripts/app-starter.sh telegram", 0)};
